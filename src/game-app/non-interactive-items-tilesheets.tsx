...
export interface NonInteractiveItemTilesheetsState {
...
interface NonInteractiveItemApiRequest {
    storyBoardState: GameStoryBoardState;
...
interface NonInteractiveItemApiResponse {
    nonInteractiveItems: String;
    imageUrl: string;
}
...
const NonInteractiveItemTilesheets: React.FC<{ state: NonInteractiveItemTilesheetsState; setState: Dispatch<SetStateAction<NonInteractiveItemTilesheetsState>>; apiKey: string }> = ({ state, setState, apiKey }) => {
    ...
    const fetchEnvironmentImage = async (environment: Environment) => {
        ...
        const apiRequest: NonInteractiveItemApiRequest = { storyBoardState: state.storyBoardState, environment, username: currentUser.username, apiKey };

        const response = await fetch('/api/generate_non_interactive_items', {
            method: 'POST',
            headers: {
        ...
        const data: NonInteractiveItemApiResponse = await response.json();
        return data;
    };
    ...
    const handleButtonClick = async (Environment: Environment) => {
        ...
        try {
            ...
            const interactiveItems = data.nonInteractiveItems;
            setState(prevState => {
    ...
    return (
        <div className="container mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4">Non Interactive Item Tilesheets</h1>
...
export default NonInteractiveItemTilesheets;